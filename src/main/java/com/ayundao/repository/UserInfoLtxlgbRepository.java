...
public interface UserInfoLtxlgbRepository extends BaseRepository<UserInfoLtxlgb,String> {
    ...
    @Query(value = "SELECT COUNT(CASE WHEN ui.`EDUCATION` = '高中' THEN 1 ELSE NULL END ) AS highSchool,\n" +
            ...
            "       FROM t_user_info ui\n" +
            "       INNER JOIN t_user_info_ltxlgb uil on ui.id = uil.userinfoid",nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(CASE WHEN ui.department = '内科' THEN 1 ELSE NULL END) AS nternal,\n" +
            ...
            "FROM t_user_info ui INNER JOIN t_user_info_ltxlgb uil on ui.id = uil.userinfoid",nativeQuery = true)
    Map<String,Object> countByDepartmentLtxlgb();

    /**
     * 查询离退休老干部详情离退休时间比例
     */
    @Query(value = "SELECT COUNT(CASE WHEN (YEAR(NOW()) - SUBSTRING(uil.`TIME`,1,4)) <=2 THEN 1 ELSE NULL END ) AS '2年以下',\n" +
            "            COUNT(CASE WHEN (YEAR(NOW()) - SUBSTRING(uil.`TIME`,1,4)) BETWEEN 2 AND 5 THEN 1 ELSE NULL END ) AS '2-5年',\n" +
            "            COUNT(CASE WHEN (YEAR(NOW()) - SUBSTRING(uil.`TIME`,1,4)) BETWEEN 5 AND 10 THEN 1 ELSE NULL END ) AS '5-10年',\n" +
            "            COUNT(CASE WHEN 10<=(YEAR(NOW()) - SUBSTRING(uil.`TIME`,1,4)) THEN 1 ELSE NULL END ) AS '10年以上'\n" +
            "            FROM t_user_info ui INNER JOIN t_user_info_ltxlgb uil on ui.id = uil.userinfoid",nativeQuery = true)
